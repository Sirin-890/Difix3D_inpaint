...
# # ─────────────────────────────────────────────────────────────────────────────
# # Teacher wrapper
# # Wraps DifixPipeline so it accepts the same (B, V, 3, H, W) tensor input
# # the training loop uses, and returns (B, V, 3, H, W) in [-1, 1].
...
#         )
#         self.pipe.to("cuda")
#         # freeze everything inside the pipeline
#         for p in self.pipe.parameters() if hasattr(self.pipe, "parameters") else []:
#             p.requires_grad_(False)

#     @torch.no_grad()
#     def forward(self, x: torch.Tensor, prompt_tokens=None) -> torch.Tensor:
#         """
#         x : (B, V, 3, H, W)  float, values in [-1, 1]
#         returns (B, V, 3, H, W) float, values in [-1, 1]
#         """
#         B, V, C, H, W = x.shape
#         outputs = []
...
#             view_outputs = []
#             for v in range(V):
#                 # convert single frame tensor → PIL for the pipeline
#                 frame = x[b, v]                          # (3, H, W)  [-1, 1]
#                 frame_01 = (frame * 0.5 + 0.5).clamp(0, 1)
#                 pil_img = transforms.ToPILImage()(frame_01.float().cpu())

#                 # DifixPipeline.from_pretrained returns images as PIL or tensors
#                 # depending on the pipeline implementation — handle both
...
#                     if out_t.max() > 1.1:           # uint8 range
#                         out_t = out_t / 255.0
#                 else:
#                     raise ValueError(f"Unexpected pipeline output type: {type(result)}")

#                 # resize back to match input resolution (pipeline may resize internally)
#                 if out_t.shape[-2:] != (H, W):
#                     out_t = F.interpolate(
#                         out_t.unsqueeze(0), size=(H, W), mode="bilinear",
#                         align_corners=False
#                     ).squeeze(0)
...
#             outputs.append(torch.stack(view_outputs, dim=0))   # (V, 3, H, W)

#         return torch.stack(outputs, dim=0)                     # (B, V, 3, H, W)
...
#     # ── Teacher (DifixPipeline, always frozen) ────────────────────────────────
#     net_teacher = None
#     if not args.no_teacher:
#         net_teacher = DifixTeacher(pretrained_name=args.teacher_repo)
#         # keep on cuda but outside accelerator — it never gets gradients
#         net_teacher.eval()
...
#         # ── pull weights out of the pipeline's internal unet/vae ─────────────
#         # DifixPipeline exposes .unet and .vae as standard diffusers modules
#         print("=" * 50)
...
#         # try to grab unet + vae state dicts from the pipeline directly
...
#         print(f"  Copied {copied} tensors, skipped {skipped} (shape mismatch / new layers).")
...
#                 # ── teacher forward (PIL-based, outside autocast) ─────────
#                 teacher_pred = None
#                 if net_teacher is not None and args.lambda_distill > 0:
#                     # run in float32 — pipeline may not support bf16/fp16
#                     with torch.no_grad():
