...
# ─────────────────────────────────────────────────────────────────────────────
# Teacher wrapper
# Wraps DifixPipeline so it accepts (B, V, 3, H, W) tensor input and returns
# (B, V, 3, H, W) in [-1, 1].  Calls pipeline with prompt + image + ref_image
# exactly as the reference example does.
# Always runs under torch.no_grad() — never receives gradients.
# ─────────────────────────────────────────────────────────────────────────────
class DifixTeacher(torch.nn.Module):
    ...
    @torch.no_grad()
    def forward(self, x: torch.Tensor, prompt: str = "") -> torch.Tensor:
        """
        x      : (B, V, 3, H, W)  float32, values in [-1, 1]
                 — the masked / corrupted multi-view input
        prompt : text prompt forwarded to the pipeline (default: "")

        returns (B, V, 3, H, W) float32, values in [-1, 1]

        For each (b, v) pair:
          - image     = x[b, v]   (the view being inpainted)
          - ref_image = x[b, 0]   (view-0 of the same scene as a clean anchor)

        Pipeline is called with:
            num_inference_steps=1, timesteps=[199], guidance_scale=0.0
        matching the reference single-step inference example.
        """
        B, V, C, H, W = x.shape
        outputs = []
        ...
        for b in range(B):
            ...
            # ── build ref PIL once per batch item (view 0 as the clean anchor) ──
            ref_frame    = x[b, 0]
            ref_frame_01 = (ref_frame * 0.5 + 0.5).clamp(0, 1)
            pil_ref      = transforms.ToPILImage()(ref_frame_01.cpu())
            del ref_frame, ref_frame_01
            ...
            for v in range(V):
                ...
                if hasattr(result, "images"):
                    ...
                    if out_t.max() > 1.1:                # uint8 range guard
                        out_t = out_t / 255.0
                else:
                    raise ValueError(
                        f"Unexpected pipeline output type: {type(result)}"
                    )
                del result

                # resize back to input resolution if pipeline changed it
            ...
            # flush pipeline intermediates after every batch item
...
# ─────────────────────────────────────────────────────────────────────────────
def main(args):
    # reduce allocator fragmentation early
    ...
    # ── Teacher (always frozen, optionally parked on CPU between steps) ───────
    net_teacher = None
    ...
    # perceptual nets in fp16 to halve VRAM footprint
    ...
    if args.resume is not None:
        if os.path.isdir(args.resume):
            ckpt_files = sorted(
                ...
                key=lambda x: int(
                    x.split("/")[-1].replace("model_", "").replace(".pkl", "")
                ),
            )
            assert len(ckpt_files) > 0
            print(f"Resuming from {ckpt_files[-1]}")
            global_step = int(
                ckpt_files[-1].split("/")[-1].replace("model_", "").replace(".pkl", "")
            )
            net_student, optimizer = load_ckpt_from_state_dict(
                net_student, optimizer, ckpt_files[-1]
            )
        elif args.resume.endswith(".pkl"):
            print(f"Resuming from {args.resume}")
            global_step = int(
                args.resume.split("/")[-1].replace("model_", "").replace(".pkl", "")
            )
            net_student, optimizer = load_ckpt_from_state_dict(
                net_student, optimizer, args.resume
            )
        else:
    ...
    t_vgg_renorm = transforms.Normalize(
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers(
            ...
            init_kwargs={
                "wandb": {"name": args.tracker_run_name, "dir": args.output_dir}
            },
        )
        ...
        for step, batch in enumerate(dl_train):
            with accelerator.accumulate(net_student):
                ...
                if net_teacher is not None and args.lambda_distill > 0:
                    ...
                    with torch.no_grad():
                        teacher_pred = net_teacher(
                            ...
                            prompt=args.prompt or "",   # text prompt
                ...
                loss_distill = torch.tensor(
                    0.0, device=accelerator.device, dtype=weight_dtype
                )
                if teacher_pred is not None:
                    teacher_flat = rearrange(
                        teacher_pred, "b v c h w -> (b v) c h w"
                    )
                    loss_distill = distillation_loss(
                        x_tgt_pred_flat, teacher_flat, mask_flat
                    ) * args.lambda_distill
                    loss = loss + loss_distill
                    del teacher_flat, teacher_pred

                loss_gram = torch.tensor(
                    0.0, device=accelerator.device, dtype=weight_dtype
                )
                if args.lambda_gram > 0 and global_step > args.gram_loss_warmup_steps:
                    ...
                    x_pred_crop = crop(
                        t_vgg_renorm(x_tgt_pred_flat * 0.5 + 0.5),
                        top, left, crop_h, crop_w,
                    )
                    x_tgt_crop = crop(
                        t_vgg_renorm(x_tgt_flat * 0.5 + 0.5),
                        top, left, crop_h, crop_w,
                    )
                    loss_gram = gram_loss(
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.viz_freq == 1:
                        ...
                        log_imgs = {
                            "train/source": [
                                wandb.Image(
                                    rearrange(x_src_b, "b v c h w -> b c (v h) w")[i]
                                    .float().detach().cpu(),
                                    caption=f"idx={i}",
                                )
                                for i in range(B)
                            ...
                            "train/target": [
                                wandb.Image(
                                    rearrange(x_tgt, "b v c h w -> b c (v h) w")[i]
                                    .float().detach().cpu(),
                                    caption=f"idx={i}",
                                )
                                for i in range(B)
                            ...
                            "train/model_output": [
                                wandb.Image(
                                    rearrange(x_tgt_pred, "b v c h w -> b c (v h) w")[i]
                                    .float().detach().cpu(),
                                    caption=f"idx={i}",
                                )
                                for i in range(B)
                            ...
                            "train/mask": [
                                wandb.Image(
                                    rearrange(mask, "b v c h w -> b c (v h) w")[i]
                                    .float().detach().cpu(),
                                    caption=f"idx={i}",
                                )
                                for i in range(B)
                    ...
                    if global_step % args.checkpointing_steps == 1:
                        ...
                        save_ckpt(
                            accelerator.unwrap_model(net_student), optimizer, outf
                        )

                    if args.eval_freq > 0 and global_step % args.eval_freq == 1:
                        # move teacher to cuda for eval if it was parked on cpu
                        ...
                        for vstep, batch_val in enumerate(dl_val):
                            ...
                            mask_v = batch_val["mask_pixel_values"].to(
                                accelerator.device, dtype=weight_dtype
                            )
                            with torch.no_grad():
                            ...
                            if vstep % 10 == 0:
                                log_dict["sample/source"].append(
                                    wandb.Image(
                                        rearrange(x_src_v, "b v c h w -> b c (v h) w")[0]
                                        .float().detach().cpu()
                                    )
                                )
                                log_dict["sample/target"].append(
                                    wandb.Image(
                                        rearrange(x_tgt_v, "b v c h w -> b c (v h) w")[0]
                                        .float().detach().cpu()
                                    )
                                )
                                log_dict["sample/model_output"].append(
                                    wandb.Image(
                                        rearrange(x_pred_v, "b v c h w -> b c (v h) w")[0]
                                        .float().detach().cpu()
                                    )
                                )
                            ...
                            mask_v0   = mask_v[:, 0]
                            l_l2.append(
                                masked_l2(x_pred_v0, x_tgt_v0, mask_v0).item()
                            )
                            l_lpips.append(
                                masked_lpips(net_lpips, x_pred_v0, x_tgt_v0, mask_v0).item()
                            )
                            del x_pred_v, x_src_v, x_tgt_v, mask_v
...
if __name__ == "__main__":
    ...
    parser.add_argument("--teacher_repo",           default="nvidia/difix_ref", type=str,
        help="HuggingFace repo id for DifixPipeline teacher.")
    parser.add_argument("--no_teacher",             action="store_true",
        help="Disable teacher entirely — train with reconstruction losses only.")
    parser.add_argument("--warmstart_from_teacher", action="store_true",
        help="Copy shape-compatible weights from teacher pipeline into student.")
